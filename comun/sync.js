...
/** POST a la hoja, con el PIN de la finca. `cuerpo` va como texto plano para no disparar preflight de CORS. */
function pedir(urlSheet, cuerpo){
  if (!urlSheet) return Promise.reject(new Error('sin-url'));
  const pin = leerPin();
  if (!pin) return Promise.reject(new Error('sin-pin'));
  return fetch(urlSheet, {
    method: 'POST',
    headers: {'Content-Type': 'text/plain;charset=utf-8'},
    body: JSON.stringify(Object.assign({}, cuerpo, {pin}))
  }).then(r => {
    ...
    if (!j.ok){
      if (j.error === 'pinInvalido') pinRechazado();
